...
check_status_cli() {
    ...
    # Only detect real active sessions (pts, notty, or @ip)
    if ps -ef | grep "sshd: $u" | grep -v grep | grep -qE "@|pts/|notty"; then
...
fun_online() {
    ...
    while IFS='|' read -r u d t n; do
        [[ -z "$u" ]] && continue
        if ps -ef | grep "sshd: $u" | grep -v grep | grep -qE "@|pts/|notty" || pgrep -u "$u" dropbear >/dev/null; then
...
def get_status(u):
    try:
        # Strict Check
        cmd = f"ps -ef | grep 'sshd: {u}' | grep -v grep | grep -E 'pts/|notty|@'"
