...
# ==================================================
#  NEW SERVER INSTALLER: SSH MANAGER BOT (V35)
#  - INSTALLS PYTHON & DEPENDENCIES
#  - SETS UP BOT & SERVICE
#  - CLEAN INTERFACE + EMOJI BUTTONS
# ==================================================
...
# --- 3. CREATE BOT SCRIPT ---
echo -e "\033[1;34m>> CREATING BOT SCRIPT...\033[0m"
                ...
                for line in lines:
                    ...
                    if d != "never":
                        try:
                            ...
                            if now >= exp:
                                kill = True
                                run_cmd(f"pkill -KILL -u {u}"); run_cmd(f"userdel -f -r {u}")
                                updater.bot.send_message(chat_id=ADMIN_ID, text=f"EXPIRED | REMOVED USER: {u}")
                        except: pass
                    if not kill:
                        try:
                            cnt = int(subprocess.getoutput(f"ps -u {u} -o stat,comm | grep -v 'Z' | grep 'sshd' | wc -l"))
                            if cnt > 1:
                                kill = True
                                run_cmd(f"pkill -KILL -u {u}"); run_cmd(f"userdel -f -r {u}")
                                updater.bot.send_message(chat_id=ADMIN_ID, text=f"CHEATER | REMOVED USER: {u}")
                        except: pass
...
def txt(update: Update, context: CallbackContext):
    ...
    if act == 'a1': context.user_data.update({'nu': msg, 'act': 'a2'}); update.message.reply_text("ENTER PASSWORD :")
        ...
        if found:
            ...
            run_cmd(f"usermod -U {u}"); update.message.reply_text("✅ RENEWED SUCCESSFULLY!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 BACK", callback_data='back')]]))
...
echo -e "\033[1;34m>> CREATING SYSTEMD SERVICE...\033[0m"
